...
## Putty 로 EC2 접속할 때 주의 점

집에서 노트북으로 사용하는경우 LAN 선을 직접 꽂거나 와이파이로 사용하는 경우가 있다. 따라서 `AWS EC2 보안그룹` 에서 인바운드 규칙 편집을 누르고 `SSH TCP 22 내 IP` 를 선택해서 IP 를 추가하면 Putty 로 접속이 잘 된다. 

## 그레이들 bootJar

Putty 에서 깃허브에 있는 프로젝트를 clone 받고 해당 프로젝트 위치로 이동해서 `ll` 명령어를 하면 `build` 폴더가 있다.

자동 배포를 위해서 쉘 스크립트를 작성할 때 build 폴더 안에 libs 폴더가 있어야 하는데 libs 폴더 안에는 실행가능한 프로젝트명으로 된 jar 파일이 있어야 한다.

생성 방법은 putty 에서 `gradlew` 가 있는 위치까지 이동하고 gradlew 는 그레이들이 설치되지 않은 환경 혹은 버전이 다른 상황에서도 해당 프로젝트에 한해서 그레이들을 쓸 수 있도록 지원하는 Wrapper 파일이다.

`./gradlew bootJar` 명령어를 실행하면 libs 폴더와 jar 파일이 생성된다.

## 직렬화와 관련된 좋은글

> https://woowabros.github.io/experience/2017/10/17/java-serialize.html

SessionUser 클래스를 만드는 이유에 대해서 정확하게 알 수 있음
